...
Outside the team layout: `/login` (Team / Client / Vendor tabs), `/portal` (client portal; team preview `/portal?lead=ID`), `/vendor` (vendor portal: Open works / My bids / Work orders), `/vendor/tenders/:id` (tender page + bid), `/vendor/register` (public vendor registration + `?track` status check), `/enquiry` (public enquiry form).
