...
import { getEmployeeProjectById } from '@/core/constants/projects';
...
export const reimbursementService = {
    getClaims: async () => {
        return new Promise(resolve => setTimeout(() => resolve(storage.getReimbursements()), 80));
    },

    getClaimById: async (id) => {
        const list = storage.getReimbursements();
        const found = list.find((r) => r.id === id || r.claimId === id);
    ...
    submitClaim: async (data) => {
        ...
        const rawAmount = data.claimAmount !== undefined ? data.claimAmount : data.amount;
        ...
        const isDuplicate = list.some((r) => 
            ...
            r.date === claimDate &&
            Number(r.claimAmount) === amount &&
            r.status !== 'Rejected' &&
            r.financeStatus !== 'Rejected'
        );

        if (isDuplicate) {
            throw new Error(`Duplicate claim detected: An allowance claim for ₹${amount.toLocaleString('en-IN')} in '${data.category}' on ${claimDate} is already pending or approved.`);
        }
        ...
        const newClaim = {
            ...
            claimAmount: amount,
            requestedAmount: amount, // standard alias
            approvedAmount: 0,
            rejectedAmount: 0,
            date: claimDate,
            ...
            auditHistory: [
                {
                    ...
                    details: `Allowance claim for ₹${amount.toLocaleString('en-IN')} submitted under ${data.category} (${resolvedProject}).`,
                }
            ],
        };
        ...
        dispatchNotification({
            type: 'Reimbursement',
            targetRole: 'hr',
            title: 'New Allowance Reimbursement Filed',
            message: `${newClaim.employeeName} filed a reimbursement claim of ₹${amount.toLocaleString('en-IN')} for ${newClaim.category}.`,
            actionLink: '/hr/reimbursement',
        });

        return new Promise(resolve => setTimeout(() => resolve(newClaim), 120));
    },

    reviewClaim: async (id, reviewData, legacyApprovedAmount, legacyRemarks) => {
        const list = storage.getReimbursements();
        ...
        let reviewerName = 'Kritika Gupta (Head HR)';
        let remarks = '';
        let targetStatus = '';

        const claimed = Number(item.claimAmount || 0);
        ...
        if (typeof reviewData === 'object' && reviewData !== null) {
            ...
            if (reviewData.approvedAmount !== undefined) {
                ...
                if (approvedAmount > claimed) {
                    throw new Error(`Approved amount (₹${approvedAmount.toLocaleString('en-IN')}) cannot exceed claimed amount (₹${claimed.toLocaleString('en-IN')}).`);
                }
                ...
                approvedAmount = targetStatus === 'Approved' ? claimed : 0;
            }
        } else if (typeof reviewData === 'string') {
            targetStatus = reviewData;
            remarks = legacyRemarks || '';
            if (legacyApprovedAmount !== undefined) {
                approvedAmount = Number(legacyApprovedAmount);
                if (isNaN(approvedAmount) || approvedAmount < 0) {
                    throw new Error('Approved amount cannot be negative.');
                }
                if (approvedAmount > claimed) {
        ...
        item.hrStatus = targetStatus || (approvedAmount === claimed ? 'Approved' : approvedAmount === 0 ? 'Rejected' : 'Partially Approved');
        ...
        if (item.hrStatus === 'Approved' || item.hrStatus === 'Partially Approved') {
            ...
            dispatchNotification({
                type: 'Reimbursement',
                targetRole: 'employee',
                title: `Allowance Claim ${item.hrStatus}`,
                message: `Claim ${item.claimId} was ${item.hrStatus.toLowerCase()} by HR for ₹${approvedAmount.toLocaleString('en-IN')}.`,
                actionLink: '/hr/reimbursement',
            });
            ...
            dispatchNotification({
                ...
                message: `Allowance claim ${item.claimId} (${item.employeeName} - ₹${approvedAmount.toLocaleString('en-IN')}) requires Finance verification.`,
            ...
            dispatchNotification({
                type: 'Reimbursement',
                targetRole: 'employee',
                title: 'Allowance Claim Rejected',
                message: `Claim ${item.claimId} was rejected by HR: ${remarks || 'Outside policy limits'}`,
                actionLink: '/hr/reimbursement',
            });
        }
    ...
    financeApprove: async (id, reviewData = {}) => {
        ...
        const remarks = reviewData.remarks || 'Audited against allowance schedule and approved for disbursement';
        ...
        dispatchNotification({
            ...
            message: `Finance verified allowance claim ${item.claimId} for ₹${Number(item.hrApprovedAmount || item.approvedAmount).toLocaleString('en-IN')}. Ready for disbursement.`,
            actionLink: '/hr/reimbursement',
        });
    ...
    financeReject: async (id, reviewData = {}) => {
        ...
        dispatchNotification({
            type: 'Finance',
            targetRole: 'employee',
            title: 'Allowance Claim Rejected by Finance',
            message: `Claim ${item.claimId} was rejected during Finance audit: ${remarks}`,
            actionLink: '/hr/reimbursement',
        });
    ...
    raiseQuery: async (id, queryData = {}) => {
        ...
        const queryId = `QRY-RMB-${Date.now().toString().slice(-6)}`;
        ...
        dispatchNotification({
            type: 'Finance',
            targetRole: 'employee',
            title: 'Clarification Required on Allowance Claim',
            message: `Finance raised a query on claim ${item.claimId}: "${queryData.message.trim()}"`,
            actionLink: '/hr/reimbursement',
        });
    ...
    respondQuery: async (id, responseData = {}) => {
        ...
        if (item.queryStatus !== 'Query Raised') {
            throw new Error(`No active query awaiting response on claim ${item.claimId} (Current query status: ${item.queryStatus}).`);
        }
        ...
        dispatchNotification({
            ...
            message: `${item.employeeName} submitted response on allowance claim ${item.claimId}. Ready for re-review.`,
            actionLink: '/finance/claims',
        });
    ...
    settleClaim: async (id, settlementData = {}) => {
        ...
        const settlementRef = typeof settlementData === 'string'
            ? settlementData
            : (settlementData?.settlementReference || `STL-RMB-${Date.now().toString().slice(-6)}`);
        ...
        try {
            ...
            if (!alreadyHasTxn) {
                await financeService.recordTransaction({
                    ...
                    status: 'Completed',
                    remarks: `Disbursal of corporate allowance claim ${item.claimId} (${item.category} - ${item.project})`,
                    date: settlementDate,
                });
                ...
                await financeService.createAccountingEntry({
                    ...
                    status: 'Posted',
                    narration: `Direct allowance reimbursement disbursement for ${item.employeeName} (${item.claimId})`,
                });
            }
        } catch (finErr) {
        ...
        item.disbursementDate = settlementDate;
        ...
        dispatchNotification({
            type: 'Finance',
            targetRole: 'employee',
            title: 'Allowance Claim Settled',
            message: `Allowance claim ${item.claimId} has been settled. ₹${payableAmount.toLocaleString('en-IN')} disbursed via ${settlementRef}.`,
            actionLink: '/hr/reimbursement',
        });
    ...
    updateClaimStatus: async (id, status, remarks, approvedAmount) => {
        return reimbursementService.reviewClaim(id, {
            status,
            remarks,
